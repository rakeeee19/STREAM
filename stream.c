...
static char	*label[2] = {"Write:     ", "Read:      "};
...
main()
	...
	BytesPerWord * ( (double) STREAM_ARRAY_SIZE / 1024.0/1024.),
	BytesPerWord * ( (double) STREAM_ARRAY_SIZE / 1024.0/1024./1024.));
...
void checkSTREAMresults ()
{
	...
	/* The write kernel stores k+1 each iteration, so the final value is NTIMES. */
	aj = (STREAM_TYPE) NTIMES;
	...
	if (err == 0) {
		printf ("Solution Validates: avg error less than %e on the write array\n",epsilon);
	}
#ifdef VERBOSE
	printf ("Results Validation Verbose Results: \n");
	printf ("    Expected a(1): %f \n",aj);
	printf ("    Observed a(1): %f \n",a[1]);
	printf ("    Rel Error on a: %e \n",abs(aAvgErr/aj));
#endif
}

#ifdef TUNED
/* stubs for "tuned" versions of the kernels */
void tuned_STREAM_Write(STREAM_TYPE value)
{
	ssize_t j;
#pragma omp parallel for
        for (j=0; j<STREAM_ARRAY_SIZE; j++)
